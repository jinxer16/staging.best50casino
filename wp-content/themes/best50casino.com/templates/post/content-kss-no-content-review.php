...
wp_reset_postdata();
?>
<div class="main-box d-flex flex-wrap w-100" id="review">
    <div class="w-70 w-sm-100 p-5p">
        <div id="ScreenshotsCarousel" class="carousel slide" data-ride="carousel">
            ...
            <ol class="carousel-indicators">
                ...
                <?php $comp2= get_post_meta($post->ID,"casino_custom_meta_comp_screen_2",true);
                if ($comp2 != ''){?>
                    <li data-target="#ScreenshotsCarousel" data-slide-to="1"></li>
                    <?php
                }
                ?>
                <?php $comp3= get_post_meta($post->ID,"casino_custom_meta_comp_screen_3",true);
                if ($comp3 != ''){?>
                    <li data-target="#ScreenshotsCarousel" data-slide-to="2"></li>
                    <?php
                }
                ?>
            </ol>
            <div class="carousel-inner">
                <div class="carousel-item active">
                    <div class="d-block w-100" style="height:400px; background-position: bottom; background-size:cover; background-image: url('<?= get_post_meta($post->ID,"casino_custom_meta_comp_screen_1",true)?>')"></div>
                </div>
                <?php
                ...
                if ($comp2meta != '') {?>
                    <div class="carousel-item">
                        <div class="d-block w-100"
                             style="height:400px; background-position: bottom; background-size:cover; background-image: url('<?= get_post_meta($post->ID, "casino_custom_meta_comp_screen_2", true) ?>')"></div>
                    </div>
                    <?php
                }
                ...
                if ($comp3meta != '') {?>
                    <div class="carousel-item">
                        <div class="d-block w-100" style="height:400px; background-position: bottom; background-size:cover; background-image: url('<?= get_post_meta($post->ID,"casino_custom_meta_comp_screen_3",true)?>')"></div>
                    </div>
                    <?php
                }
                ?>
            </div>
            <div class="w-20 d-flex pt-20p pl-3p pb-15p flex-column position-absolute top-0">
                <img  src="<?= get_post_meta($post->ID,"casino_custom_meta_trans_logo",true);?>" loading="lazy" class="img-fluid  d-block mx-auto"  style="max-width: 140px; max-height: 140px;">
...
<div class="main-box d-flex flex-wrap w-100">
    ...
    <?php
    $query_promo = array( // A QUERY that initializes the default (all) IDS
    ...
    $query_promos = get_posts($query_promo);

    $countoffer = count($query_promos);
    ...
    <div class="w-70 w-sm-100 p-5p">
        ...
        ?>
        <div class="offer-shortcode d-flex flex-wrap w-100 p-5p w-sm-100" id="bonus">
            <?php
            foreach ($query_promos as $promoid){
                ...
                $ctapromo = get_post_meta($promoid,'promo_custom_meta_cta_aff',true) ?  get_post_meta($promoid,'promo_custom_meta_cta_aff',true): $ctaLink;
                $ctatext = get_post_meta($promoid,'promo_custom_meta_cta',true) ?  get_post_meta($promoid,'promo_custom_meta_cta_aff',true): 'Claim Offer';
                $minm_depo= get_post_meta($promoid,'promo_custom_meta_min_depo',true) ?  get_post_meta($promoid,'promo_custom_meta_min_depo',true): '';
                $bonusCode= get_post_meta($promoid,'promo_custom_meta_bonus_code',true) ?  get_post_meta($promoid,'promo_custom_meta_bonus_code',true): '';
                $terms = get_the_terms($promoid, 'promotions-type');
                ?>
                <div class="d-flex <?=$offerwidth?> flex-wrap p-5p">
                    <div class="w-100 m-5p d-flex flex-wrap p-5p border">
                        <span class="font-weight-bold d-block w-100 pl-5p pr-5p pb-2p text-dark text-21"><?=get_the_title($promoid)?></span>
                        <span class="font-weight-bold d-block w-100 pl-5p pr-5p text-success text-18"><i class="fas fa-gift"></i> <?=$terms[0]->name;?> Promotion</span>

                        <div class="w-70 align-self-center d-flex flex-wrap">
                            <div class="w-100 mb-3p text-13 bg-gray-light d-block p-5p border font-weight-bold"><i class="fas fa-minus-square text-16"></i> Min  Deposit :<?=$minm_depo?></div>
                            <div class="w-100 mb-3p text-13 bg-gray-light d-block p-5p border font-weight-bold"><i class="fas fa-tags text-16"></i> Bonus Code :<?=$bonusCode?></div>
                            <div class="w-100 mb-3p text-13 bg-gray-light d-block p-5p border font-weight-bold"><i class="fas fa-stopwatch text-16"></i> Bonus Expiration :<span data-title="Countddown" class="countdown font-weight-bold text-center text-12 p-5p" data-time="<?= $offerEndTime?>"></span></div>
                        </div>
                        <div class="w-30 align-self-center">
                            <a class="btn bg-yellow text-13 w-80 d-block mb-5p mt-5p mx-auto p-5p btn_large text-dark text-decoration-none font-weight-bold bumper" style="box-shadow: 0 5px 15px rgba(145, 92, 182, .4);"  data-casinoid="<?php echo $post->ID; ?>" data-country="<?php echo $countryISO; ?>"  <?php echo $ctaFunction; ?> href="<?php echo $ctapromo?>" rel="nofollow" target="_blank">
                                <span><?=$ctatext?></span>
                            </a>
                        </div>

                        <div class="w-100 text-11 mt-5p mb-5p bg-gray-light d-block p-5p border font-weight-bold"><i class="fas fa-info-circle text-16"></i><?=get_post_meta($promoid,'promo_custom_meta_tcs',true)?></div>
                    </div>
                </div>
                <?php
            }
        ...
        <div class="w-70 w-sm-100 p-5p">
            ...
            <div class="section-ratings mb-10p  p-5p shadow-box  mt-10p" id="payout">
                <div class="d-flex flex-wrap w-100">
                    <div class="w-100 d-flex flex-wrap p-15p">
                        <div class="w-20">
                        </div>
                        <p class="w-80 d-block text-20">Payout Rates of Casino Games</p>
                        <div class="w-20 pr-10p games-rat-title">
                            <div class="mb-10p d-flex flex-wrap title-game">
                                <div class="w-80">
                                    Slots
                                </div>
                                <div class="w-20">
                        ...
                        <div class="w-70 payout-rates" style="border-left: 1px solid #ab8f8f; border-bottom: 1px solid #ab8f8f;">

                            <div class="progress w-100 mb-10p ">
                                <?php
                                $value =get_post_meta($post->ID,"casino_custom_meta_slots_rtp",true) ;
                                $progressbar='';
                                if ($value == 0){
                                    $progressbar='empty-progress';
                                }else{
                                    $progressbar='';
                                }
                                ?>
                                <div class="<?=$progressbar?> progress-bar text-black font-weight-bold" role="progressbar" aria-valuenow="<?php if ($value ==0){echo 5;}else{echo $value;}  ?>" aria-valuemin="0" aria-valuemax="100">
                                    <?php echo $value; ?> %
                                </div>
                            </div>
                            <div class="progress w-100 mb-10p">
            ...
            <div class="section-withdrawls mt-10p mb-10p" id="banking">
        ...
        ?>
            <ul class="nav nav-tabs pl-0 mb-0" id="myTab" role="tablist">
                <li class="nav-item w-25 text-center pl-0 list-none">
                    <a class="nav-link active font-weight-bold text-decoration-none p-15p" id="depo-tab" data-toggle="tab" href="#depotab" role="tab" aria-controls="depotab" aria-selected="true">Deposit Methods</a>
                </li>
                <li class="nav-item w-25 text-center pl-5p list-none">
                    <a class="nav-link font-weight-bold text-decoration-none p-15p" id="with-tab" data-toggle="tab" href="#withtab" role="tab" aria-controls="withtab" aria-selected="false">Withdrawal Options</a>
                </li>
            </ul>
            <div class="tab-content" id="myTabContent">
                <div class="tab-pane fade show active" id="depotab" role="tabpanel" aria-labelledby="depotab">
                    <div class="d-flex flex-wrap w-100">
                        <div class=" w-20 p-7p text-center font-weight-bold text-14" style="color: #354046;background: #c7ccce;">Deposit Methods</div>
                        <div class=" w-20 p-7p text-center font-weight-bold text-14" style="color: #354046;background: #c7ccce;">Deposit Minimum</div>
                        <div class=" w-20 p-7p text-center font-weight-bold text-14" style="color: #354046;background: #c7ccce;">Deposit Maximum</div>
                        <div class=" w-20 p-7p text-center font-weight-bold text-14" style="color: #354046;background: #c7ccce;">Deposit Time</div>
                    ...
                    <div class="d-flex flex-wrap w-100">
                        ...
                        foreach ($depArrayFirst as $rest){
                            ...
                            <div class="d-flex flex-wrap w-100 p-7p deporows">
                                <div class="w-20 text-left align-self-center">
                                ...
                                <div class="w-20 align-self-center text-center align-middle">
                                    ...
                                    <div class="d-flex flex-wrap hidden-payments deporows w-100 p-7p">
                                        <div class="w-20 text-left align-self-center">
                                            <div class="d-flex flex-wrap">
                                                ...
                                                <div class="w-60 align-self-center text-left pl-5p">
                                                    <?= get_the_title($rest);?>
                                                </div>
                                            </div>
                                        </div>
                ...
                <div class="tab-pane fade" id="withtab" role="tabpanel" aria-labelledby="withtab">
                    <div class="d-flex flex-wrap w-100">
                        <div class=" w-20 p-7p text-center font-weight-bold text-14" style="color: #354046;background: #c7ccce;">Withdrawal Methods</div>
                        <div class=" w-20 p-7p text-center font-weight-bold text-14" style="color: #354046;background: #c7ccce;">Withdrawal Minimum</div>
                        <div class=" w-20 p-7p text-center font-weight-bold text-14" style="color: #354046;background: #c7ccce;">Withdrawal Maximum</div>
                        <div class=" w-20 p-7p text-center font-weight-bold text-14" style="color: #354046;background: #c7ccce;">Withdrawal Time</div>
                        <div class=" w-20 p-7p text-center font-weight-bold text-14" style="color: #354046;background: #c7ccce;"></div>
                    </div>
                    <div class="d-flex flex-wrap w-100">
                        <?php
        ...
        <div class="d-block w-100 mt-5p" id="comments">
    ...
    <div class="w-30 w-sm-100  p-5p">
        ...
        <div class="widget2 mt-5p ">
            <span class="widget2-heading bg-dark text-left">Game Providers</span>
            <?php $deposits = get_post_meta($post->ID, 'casino_custom_meta_softwares', true) ?>
            <div class="review-box-body pb-10 pt-10 d-flex flex-wrap  shadow">
                <?php
                if($deposits){
                    foreach ($deposits as $payID){
                        echo '<div class="soft-box p-1p">
                                    <img src="'.get_post_meta($payID, 'casino_custom_meta_sidebar_icon', true).'" class="img-fluid w-100 overflow-hidden d-block text-center p-5p soft-img" alt="'.get_the_title($payID).'" loading="lazy">
                                    <span class="text-12 d-block text-center text-dark">'.get_the_title($payID).'</span>
                                    </div>';
                    }
                    wp_reset_postdata();
                }?>
        ...
        <div class="widget2 position-sticky" style="top: 12%;" id="bestCasinos">
            ...
            foreach ($cas as $casinoID){
                ...
                $bonusNamebest = get_post_meta($bonuspagebest, 'bonus_custom_meta_bonus_offer', true);
