<?php
function add_promotions_shortcode($atts)
{
    $atts = shortcode_atts(
        array(
            'casino' => '',
    ...
    if ($atts['casino'] != 'all' && isset($atts['casino']) ) {
        $casinoQuery = array(
            array(
                'key' => 'casino_offer_addtional',
                'compare' => 'LIKE',
                'value' => serialize($atts['casino']),
    ...
    if(!empty($query_promos)){
        foreach ($query_promos as $promoID) {
            ...
            if (!in_array($GLOBALS['countryISO'], get_post_meta($casinoMainID, 'casino_custom_meta_rest_countries', true)) && get_post_meta((int)$promoID, 'promo_custom_meta_end_offer', true) >= $dttest) {
                ...
                $ret .= '<div class="col-lg-2 col-xl-2 col-md-2 col-6 order-1 casino-logos align-self-start p-0 p-sm-5p" ><img src="' . get_the_post_thumbnail_url($casinoMainID) . '" loading="lazy" class="h-auto w-100 img-fluid"  alt="' . get_the_title($casinoMainID) . '"></div>';
...
add_shortcode('add_promo', 'add_promotions_shortcode');