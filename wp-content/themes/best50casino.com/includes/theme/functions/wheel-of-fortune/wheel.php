...
ob_start();
    ...
    foreach ($pop as $popID){
        $fortunes = get_post_meta($popID,'fortune_gifts_meta',true);
        if (isset($fortunes) ) {
            $arraybgPiece= array();
            foreach ( $fortunes as $value ) {
                $bg_piece = $value['bg_piece'] ?? false;
                $arraybgPiece[] = $bg_piece;
            }
        }
        $buttontext = get_post_meta($popID,'button_text',true);
        $terms = get_post_meta($popID,'terms',true);
    }

    $date = date('Y-m-d');//2021/06/03
    $timestamp = strtotime($date);
    $day = date('l', $timestamp);
    $data = get_latest_offers($date, $day, false, true, 999, null,'all', 'all');
    $orderPromoSetingsArray = array_slice($data['offers'], 0, 8);
    ...
    foreach ($orderPromoSetingsArray as $offerID) {
        ...
        $arraygifts[] = get_post_meta($casID, 'casino_custom_meta_trans_logo', true);
        $ctaLink = get_post_meta($offerID, 'promo_custom_meta_cta_aff', true) ?: get_post_meta($casID, 'casino_custom_meta_affiliate_link_bonus', true);
        $arrayUrls[] = $ctaLink;
    }
    $encoded = implode(",",$arraygifts);
    $encodedgi = implode(",",$arrayUrls);
    $bgpiece = implode(",",array_slice($arraybgPiece, 0, count($orderPromoSetingsArray)));
?>
...
<a class="position-absolute closewheel" style="right: 0;  border-radius: 50%;top: -6%; padding: 5px;padding-left: 6px;line-height: 1; width: 25px;background: #e5b800;height: 25px;" data-toggle="collapse" href="#fortwheel" role="button" aria-expanded="false" aria-controls="fortwheel"><i class="fa fa-close text-dark"></i></a>
 <input type="hidden" id="arrayGifts" value="<?php echo $encoded?>">
    <input type="hidden" id="arraUrl" value="<?php echo $encodedgi?>">
    <input type="hidden" id="arrayColor" value="<?php echo $bgpiece?>">
    <style type="text/css"><?php include  get_template_directory().'/includes/theme/functions/wheel-of-fortune/assets/styles.css'; ?></style>

    <div id="wheel" class="w-100 position-relative">
        <canvas class="position-absolute" id="confetti" style="z-index: 500;"></canvas>
        <span class="text-white text-center d-block d-md-none d-lg-none d-xl-none font-weight-bold titlefortunemobile"> SPIN THE <span class="text-secondary">WHEEL</span></span>
        ...
        <span class="text-white text-center mt-10p d-none d-md-block d-lg-block d-xl-block font-weight-bold titlefortune"> SPIN THE <span class="text-secondary">WHEEL</span></span>
        <span class="text-white d-block m-auto text-14 mt-10p text-center"><?php echo $terms?></span>
        <div class="button-short-cta-wrapper button threed btn_in_modal m-auto borderanimation">
            <a class="button-short-cta" style="display: flex;justify-content: center;background: linear-gradient(181deg, #00777a 0%, #00777a 35%, #343434 100%) !important; border-radius:5px !important;" target="_blank" rel="nofollow noopener">
                <span></span>
                <span></span>
                <span></span>
                <span></span>
                <div class="font-weight-bold align-items-center" style="display: flex;">
                    <?php echo $buttontext?>
                </div>
            </a>
        </div>
    ...
    <script defer type="text/javascript" src="<?php echo get_template_directory_uri();  ?>/includes/theme/functions/wheel-of-fortune/assets/js/script.js?v=21.5"></script>
