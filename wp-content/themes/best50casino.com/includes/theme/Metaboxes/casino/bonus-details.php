...
//        $iso.$prefix.'_is_mobile_bonus'.",".$iso.$prefix.'_is_welcome_bonus'.",".$iso.$prefix.'_is_reload_bonus'.",".$iso.$prefix.'_is_live_bonus'.",".
//        $iso.$prefix.'bonus_type'.",".$iso.$prefix.'spins_type'.",".$iso.$prefix.'cashback_type'.",".$iso.$prefix.'min_dep'.",".$iso.$prefix.'nodep'.",".$iso.$prefix.'sp_terms'.",".
//        $iso.$prefix.'bc_code'.",".$iso.$prefix.'cta_for_top'.",".$iso.$prefix.'cta_for_top_2'.",".$iso.$prefix.'bc_perc'.",".$iso.$prefix .'wag_b'.",".$iso.$prefix.'wag_d'.",".$iso.$prefix.'wag_s';
//}
//
...
//$fieldsToUpdate = ['exclusive','no_bonus','no_bonus_code','_is_free_spins','_is_no_dep','_is_vip','promo_code', 'bonus_type','spins_type','cashback_type','min_dep','nodep','sp_terms','bc_code','cta_for_top','cta_for_top_2','bc_perc','wag_b','wag_d', 'wag_s','bonus_contries_filled'];
    ...
    <?php foreach ($activeCountriesWithNames as $iso => $name) {
        ...
        <div class="col-12 p-0 country-bonus-section my_meta_control metabox" id="my_meta_control_country_<?= $iso ?>" data-country="<?= $iso?>">
            ...
            <h4 class="w-100 bg-primary m-0 border-bottom d-flex align-items-center"><img
                    src="<?= get_template_directory_uri() . '/assets/flags/' . $iso ?>.svg" width="20" class="ml-1 mr-1"<a class="text-white p-2 d-block" data-toggle="collapse"
            ...
            <div class="panel-collapse collapse" id="details-<?= $iso ?>">
                <div class="d-flex flex-wrap">
                    ...
                    <div class="col-2 p-3 d-flex align-items-center">
                        ...
                        <input type="checkbox" name="<?php $mb->the_name(); ?>" id="no_bonus" onclick="noBonus(this,'nobonus','<?echo $iso;?>')" value="1"<?php $mb->the_checkbox_state('1'); ?> class="ml-1"/>
                    </div>
                    <div class="col-2 p-3 d-flex align-items-center">
                        <label class="mb-0">No Bonus Code</label>
                        <?php $mb->the_field($iso . $prefix . 'no_bonus_code'); ?>
                        <input type="checkbox" name="<?php $mb->the_name(); ?>" id="no_bonus_code" onclick="noPromoCode(this,'nopromocode','<?echo $iso;?>')" value="1"<?php $mb->the_checkbox_state('1'); ?> class="ml-1"/>
                    </div>
                    <div class="col-2 p-3 d-flex align-items-center">
                    ...
                    <div class="col-3 p-3 d-flex align-items-baseline">
                        ...
                        <p class="mb-0">
                            ...
                            <input class="w-100" type="text" name="<?php $mb->the_name(); ?>" data-default="-" data-required="true" value="<?php $mb->the_value(); ?>"/>
                            <i>100% up to $500</i>
                        </p>
                    </div>
                    <div class="col-3 p-3 d-flex align-items-baseline">
        ...
        <div class="d-flex flex-wrap countries-filled-wrap">
            ...
            foreach ($activeCountriesWithNames as $iso => $name) { if(isset($casinoRestrictions[$iso]))continue;?>
                <p class="mb-0 d-flex mr-1" style="width:12%" data-countryFill="<?= $iso ?>">
                    <input type="checkbox" name="<?php $mb->the_name(); ?>" value="<?= $iso; ?>" <?php $mb->the_checkbox_state($iso); ?> hidden/>
                    <a class="w-80 <?php echo $mb->get_the_checkbox_state($iso)!=null ? 'font-weight-bold':''; ?>" href="#section-<?= $iso ?>"><?= ucwords($name) ?> </a>
                </p>
            <?php } ?>
        </div>
    </div>
